...
### 6.7.3 FlashAttention V2：面向 Tensor Core 的并行优化

FlashAttention V1 通过分块和在线 softmax 解决了注意力计算的显存瓶颈，但其循环顺序和并行策略并未充分发挥现代 GPU（尤其是 A100 及后续架构）中 Tensor Core 的峰值吞吐能力。V2 在保持 V1 核心思想的基础上，对算法进行了深度重构，实现了 **更高效的并行** 与 **更低的非矩阵乘法开销**。

**核心改进：交换循环顺序**

V1 采用“外循环遍历 $K, V$ 块，内循环遍历 $Q$ 块”的策略，即每次加载一块 $K_j, V_j$ 后，依次处理所有 $Q_i$ 块。这种方式虽然减少了 $K, V$ 的读取次数，但导致大量 softmax 统计量更新和输出重缩放操作穿插在矩阵乘法之间，打断了 Tensor Core 的流水线。

V2 将循环顺序 **完全对调**，改为 **外循环遍历 $Q$ 块，内循环遍历 $K, V$ 块**。其计算流程如下：

```text
# 初始化
for each Q block i:
    load Q_i                       # 从 HBM 加载到 SRAM
    m_i = -inf
    l_i = 0
    O_i = 0
    ...
    for each K, V block j:
        if causal and j > i:
            continue
        load K_j, V_j              # 从 HBM 加载到 SRAM
        S_ij = Q_i @ K_j^T         # 矩阵乘法，利用 Tensor Core
        ...
        l_i = exp(m_prev - m_i) * l_i + rowsum(exp(S_ij - m_i))
        O_i = exp(m_prev - m_i) * O_i + exp(S_ij - m_i) @ V_j
...
1. **减少非矩阵乘法（non-matmul）操作**  
   在 V1 中，每处理一个 $K, V$ 块就需要对所有 $Q$ 块进行统计量更新和输出重缩放，导致大量标量操作（如指数、除法）密集穿插。V2 将外循环放在 $Q$ 块上，每个 $Q_i$ 块只需在遍历完所有 $K, V$ 块后进行一次最终归一化，中间更新次数从 $T_q \times T_{kv}$ 降为 $T_q$，非矩阵乘计算量显著降低。

2. **最大化 Tensor Core 利用率**  
   $Q_i K_j^T$ 是一系列连续的矩阵乘法（批次大小 = batch × heads），V2 的内循环使得这些乘法可以 **批量提交** 给 Tensor Core，避免了因频繁切换任务而导致的流水线停顿。在 A100 上，Tensor Core 的实际占用率从 V1 的约 40% 提升至 60% 以上。

3. **增加序列维度的并行度**  
   V1 的并行化主要基于 batch 和 heads 维度，当 batch 较小时，GPU 的流式多处理器（SM）无法完全占满。V2 进一步在 **序列长度维度** 引入并行：通过将 $Q$ 分块并分配给不同的线程块，可以同时处理多个 $Q_i$ 块。这种“batch × heads × 序列块”的三级并行策略，使 SM 利用率在长序列场景下显著提升。
...
FlashAttention V2 通过交换循环顺序、提升序列维度并行度以及优化 Tensor Core 使用，在保持 V1 内存友好特性的前提下，将注意力计算的速度推向了新的高度。这一改进使得长上下文 Transformer 的训练和推理在单卡上变得更加经济高效，成为后续众多长文本模型（如 Llama 2、GPT-4 等）工程实现中的关键组件。

### 6.7.4 FlashAttention V3：面向 H100 的异步与低精度优化

FlashAttention V2 在 A100 等 Ampere 架构 GPU 上已接近 Tensor Core 的理论极限，但 H100（Hopper）架构引入了革命性的新特性：**异步执行模型**（支持计算与数据搬运完全重叠）、**WGMMA（Warpgroup Matrix Multiply-Accumulate）指令** 以及 **原生 FP8 支持**。V3 正是针对这些硬件特性进行深度优化的产物，其目标是让注意力计算在 H100 上实现 **接近理论峰值的吞吐量**，并充分利用低精度格式带来的算力红利。

**核心改进：异步 WGMMA 流水线**

V3 在 V2 的“Q 外循环、K/V 内循环”基础上，将计算流程重构为 **生产者-消费者流水线**。其核心思想是利用 H100 的异步指令，使数据加载与矩阵乘法完全并行，消除 GPU 流式多处理器（SM）因等待数据而产生的空闲周期。

```text
# 异步流水线伪代码
for each Q block i:
    load Q_i asynchronously

    for each K, V block j:
        # 异步加载下一批 K_j, V_j（生产者）
        if next block exists:
            async_load(K_{j+1}, V_{j+1})

        # 对当前已加载的 K_j, V_j 执行 WGMMA 矩阵乘法（消费者）
        # WGMMA 与异步加载同时进行
        S_ij = wgmma(Q_i, K_j)

        # 在线 softmax 更新（利用 Tensor Core 加速）
        update m_i, l_i, O_i
...
**WGMMA（Warpgroup Matrix Multiply-Accumulate）** 是 H100 引入的新型指令，它将一组 warp（32 个线程）组织为 warpgroup（4 个 warp，共 128 线程），并在硬件层面直接支持异步执行。V3 将 $Q_i K_j^T$ 的矩阵乘法拆分为多个 WGMMA 指令，这些指令在 Tensor Core 中执行的同时，CPU 可以继续发射后续的数据加载指令，实现计算与数据传输的 **完全重叠**。通过精心设计流水线阶段（通常为 2~3 级），V3 使得 SM 的利用率从 V2 的约 60% 提升至 80% 以上。

**FP8 低精度支持与混合精度策略**

H100 的第四代 Tensor Core 在 FP8 精度下的理论吞吐量是 FP16 的两倍（例如 H100 SXM 的 FP8 峰值约为 1979 TFLOPS，而 FP16 为 989 TFLOPS）。V3 原生支持 FP8 输入，但在注意力计算中必须解决 **数值稳定性** 问题，因为 softmax 对精度敏感。

V3 采用 **混合精度策略**：

- **矩阵乘法 $QK^T$** 使用 FP8 执行，充分利用 Tensor Core 的高吞吐。
- **累加器** 保持 FP16 或 BF16 精度，避免 FP8 累加时的精度损失。
- **Softmax 计算** 提升到 FP32 进行，保证指数运算的数值稳定性（softmax 中的指数和除法极易在低精度下溢出）。
- **输出 $PV$** 可选择性转换为 FP8，以适应后续层。
...
更重要的是，V3 对寄存器使用进行了精细控制，**避免寄存器溢出**（register spilling）到本地内存（L1 缓存）。通过将中间统计量（$m_i, l_i, O_i$ 的部分累加）尽量驻留在寄存器中，V3 减少了不必要的内存访问，进一步提升了流水线效率。
...
FlashAttention V3 是算法与硬件深度协同设计的典范。它通过 **异步 WGMMA 流水线** 解耦了数据加载与计算，利用 **FP8 混合精度** 释放了 H100 的低精度算力，并通过精细的 **块布局与寄存器优化** 消除了内存瓶颈。这一系列改进使得注意力计算不再是 Transformer 长上下文扩展的阻碍，而是成为能够充分利用硬件极限的“高性能组件”。V3 的诞生，直接支撑了当前工业界 100k~1M 上下文窗口的大语言模型的工程实现。
...
沐曦股份是国产GPU领域的一支重要力量，2025年营收已突破16亿元，并正在从技术验证走向规模化商业落地。

沐曦目前的主力产品是**曦云C系列**训推一体GPU，同时下一代产品已在路上：

| 产品型号 | 定位 | 关键性能对标 | 状态 |
| :--- | :--- | :--- | :--- |
| **曦云C500系列** | AI训练/推理 | 对标英伟达**A100** | 已量产，2025年主力产品 |
| **曦云C600系列** | AI训练/推理 | 介于A100和**H100**之间 | 2025年底风险量产，2026上半年正式量产 |
| **曦云C700系列** | 下一代旗舰 | 对标英伟达**H100** | 2025年4月立项，预计2026年下半年流片 |

### 6.9.2 百度昆仑芯

百度昆仑芯是国内最早涉足AI芯片领域的玩家之一，其前身是百度内部2011年成立的智能芯片及架构部，可以说是国产AI芯片的"老兵"，与华为昇腾走"系统架构弥补单点工艺"的路线、壁仞专注GPGPU不同，昆仑芯的差异化优势在于**背靠百度的真实业务场景打磨，已在内部大规模验证后走向外部市场**。

| 产品型号 | 定位与场景 | 关键算力 | 制程工艺 | 状态/上市时间 |
| :--- | :--- | :--- | :--- | :--- |
| **昆仑芯P800** | 当前主力产品，AI训练与推理 | **345 TFLOPS (FP16)** | 7nm | 2024年上市，已大规模部署 |
| **昆仑芯M100** | 大规模推理场景，极致性价比 | 未公开 | 未公开 | **2026年上市** |
| **昆仑芯M300** | 超大规模多模态模型训练与推理 | 未公开 | 未公开 | **2027年上市** |
| **天池256超节点** | 256卡互联集群 | 整体性能提升50%以上 | - | **2026年上市** |
| **天池512超节点** | 512卡互联集群 | 单节点可完成万亿参数训练 | - | **2026年上市** |

> **性能对比**：昆仑芯P800的FP16算力达345 TFLOPS，是英伟达H20的**2.3倍**。单机8卡配置下，DeepSeek一体机可提供2 PFLOPS (FP16)的峰值算力。

**出货量排名靠前**

根据IDC发布的**2024**年中国加速计算芯片出货报告：
- 英伟达：超190万片（70%份额）
- 华为昇腾：64万片（国产第一）
- **昆仑芯：6.9万片（国产第二）**
- 天数智芯：3.8万片
...
预装**百度飞桨(PaddlePaddle)** 框架，支持**TensorFlow、PyTorch**无缝迁移，提供"类CUDA"编程环境，降低迁移成本。

**2. 已点亮三万卡集群**

百度智能云今年已点亮基于昆仑芯P800的**三万卡集群**，可同时支撑多个千亿参数大模型训练，是国产AI算力集群的重要里程碑。未来规划将集群规模从三万卡扩展至**百万卡级别**。

**3. 商业化落地成果**

昆仑芯已从百度内部自用成功切入外部市场，覆盖上百家客户：

| 行业 | 客户/项目 | 应用成果 |
| :--- | :--- | :--- |
| **运营商** | 中国移动 | 2025-2026年推理型设备集采中，斩获**十亿级订单**，三个标包均获第一 |
| **金融** | 招商银行 | 仅需32台服务器即可完成千亿参数模型全参数训练 |
| **能源** | 南方电网 | AI算力支撑，提升业务智能化水平 |
| **制造** | 吉利汽车 | 智能制造场景应用 |
| **科研** | 中国钢研、国家管网 | 科研计算与AI应用 |

### 6.9.3 海光信息

海光信息是一家以**X86 CPU起家、DCU（深度计算单元）后来居上**的国产算力企业。它的独特之处在于，CPU和DCU两条产品线都达到了市场主流水准，并形成了“CPU+DCU”异构协同的核心竞争力。

**海光信息技术股份有限公司**成立于2014年，2022年8月在科创板上市。公司总部位于天津，专注于高端处理器的研发、设计和销售。与许多纯AI芯片公司不同，海光拥有两条完整的产品线：

**海光CPU**：基于x86架构，主要应用于服务器、工作站，兼容庞大的x86生态系统
**海光DCU**：深度计算单元，基于GPGPU架构，专为AI训练与高性能计算设计

这种“双芯”布局使其既能覆盖通用计算市场，又能切入AI算力赛道。

#### 核心产品：DCU系列

海光DCU是其AI算力的核心，已完成三代产品迭代。

| 产品型号 | 制程工艺 | 关键算力(FP16) | 显存/带宽 | 主要特点 |
| :--- | :--- | :--- | :--- | :--- |
| **深算一号** | 7nm | 约90 TFLOPS | 32GB HBM2 | 2021年商用，首代GPGPU产品 |
| **深算二号** | 7nm+ | 约180 TFLOPS | 512GB / 1.5TB/s | 2023年发布，性能对标A100的80%-90% |
| **深算三号BW1000** | 5nm | 400+ TFLOPS | 64GB HBM2e | **当前旗舰**，性能约为H800的50% |

>据行业测试，海光BW1000在单卡可用性能上实测达到对标产品的87%左右。更惊人的是，在CAE仿真场景中，256块DCU的并行计算速度超过4116节点13万CPU核，取得**不小于700倍的加速效果**。

#### 软件生态：类CUDA与无缝迁移

海光DCU的软件策略非常务实——**兼容现有生态，降低迁移成本**。

海光DCU采用GPGPU架构，兼容“类CUDA”编程环境，支持PyTorch、TensorFlow等主流AI框架，并且已集成超过2000个算子，对标CUDA的**算子覆盖度超过99%**，自研的DCU Toolkit支持HIP接口转换，可将CUDA代码迁移至海光平台，**迁移成本降低70%以上**，并且已与国内外主流大模型全面适配，**DeepSeek新模型发布当天即可实现无缝适配与深度调优**。

#### 实际应用案例

海光DCU已在多个关键行业实现规模化落地：

| 行业 | 应用场景 | 客户/合作伙伴 | 成果 |
| :--- | :--- | :--- | :--- |
| **金融** | AI信创改造、智能文档解析 | 某全国性股份行 | OCR识别效果达英伟达同等水平，已部署于信贷审核、尽调分析等场景 |
| **金融** | AI风控系统 | 中科金财 | 实时交易分析延迟低于10ms，较CPU方案提速15倍 |
| **科研** | 智算算力网、大模型研发 | 信息高铁平台 | 率先上线BW1000算力集群，支撑50+行业大模型研发 |
| **航空** | 国产智算平台、大模型一体机 | 紫光恒越 | 交付搭载“海光四号CPU+BW1000 GPU”的一体机，航空领域首单落地 |
| **智能制造** | 工业缺陷检测 | 澎峰科技 | 单卡并行处理32路4K视频流，检测精度达99.5% |

### 6.9.4 壁仞科技

壁仞科技是中国领先的通用GPU（GPGPU）企业，以**首款突破PFLOPS级别算力的国产芯片BR100**一鸣惊人，是国内少数具备千卡集群落地能力和异构混训技术的AI算力提供商。

**壁仞科技（上海壁仞科技股份有限公司）** 成立于2019年9月9日，总部位于上海。公司专注于开发通用图形处理器（GPGPU）芯片及智能计算解决方案，为AI大模型的训练和推理提供基础算力支撑。

**2026年1月2日**，壁仞科技在香港联交所主板上市 ，募资规模达7.17亿美元。截至2026年3月，公司拥有员工792人，累计申请专利超1500项，专利授权600余项。

####  1. 核心产品与技术

**BR100系列**

壁仞科技于2022年8月发布的首款通用GPU芯片BR100，在发布时创下多项纪录：

| 技术维度 | 参数与指标 |
| :--- | :--- |
| **制程工艺** | 7nm，集成**770亿**晶体管 |
| **封装技术** | Chiplet芯粒 + 2.5D CoWoS先进封装 |
| **峰值算力** | **PFLOPS级别**（1PFLOPS=1000万亿次），INT8达1024 TOPS，BF16达512 TFLOPS |
| **显存与缓存** | 64GB HBM2E + 超300MB片上缓存 |
| **互联协议** | PCIe 5.0、CXL、高速片间互联 |

**产品系列**

| 产品系列 | 定位与特点 | 商业化状态 |
| :--- | :--- | :--- |
| **壁砺106/110** | 基础款通用GPU芯片 | 已规模部署 |
| **壁砺166** | 性能更高的旗舰级产品 | 已规模部署，支撑千卡集群 |
| **BR20X** | 下一代旗舰芯片 | 预计2026年商业化 |

#### 2. 软件生态：BIRENSUPA平台与异构混训

壁仞科技构建了完整的软件栈 **BIRENSUPA™**，提供与CUDA风格类似的C/C++编程接口和运行时API，大幅降低代码迁移成本。

**核心技术突破：异构GPU混训**

壁仞科技自主研发了**中国首个支持四种及以上异构GPU协同训练**同一个大模型的解决方案，已实现数千卡规模混训，**联合中国移动**发布“芯合”异构混合并行训练系统，**联合中国电信、中兴通讯**发布“智算异构四芯混训解决方案”，**牵头推动**智算集群异构混训标准建设。

这一技术有效解决了国内智算中心因多品牌芯片共存而形成的“算力孤岛”难题，提升了国产算力资源的利用效率。

### 6.9.5 天数智芯

天数智芯是国内首家实现通用GPU量产的企业，产品已覆盖“云、边、端”全场景，累计出货量突破5.2万片。

#### 1. 核心产品

[天数智芯目前拥有三大产品系列，覆盖云端训练、云端推理和边端算力场景](https://zhidx.com/news/36566.html)：

| 产品系列 | 定位 | 型号 | 关键算力 | 显存 | 售价 | 主要特点 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **天垓系列** | AI训练 | 天垓100 | 147 TFLOPS @ FP16 | 32GB HBM2 | 约5万元 | 国内首款量产训练GPU，支持FP32/FP16/BF16/INT8等多精度 |
| | | 天垓150 | 192 TFLOPS @ FP16 | 64GB HBM2 | 约9万元 | 性能升级版，显存翻倍 |
| **智铠系列** | AI推理 | 智铠50 | 192 TOPS @ INT8 | 16GB HBM2e | 约1.5万元 | 低功耗，板级功耗75W |
| | | 智铠100 | **384 TOPS** @ INT8 | 32GB HBM2e | 约2.3万元 | 主力推理产品，支持FP32/FP16/INT8多精度 |
| | | 智铠150 | 300 TOPS @ INT8 | 32GB HBM2e | 约3万元 | 高性能推理 |
| **彤央系列** | 边端算力 | TY1000~TY1200 | 100~**300 TOPS**（实测稠密算力） | — | 未公开 | 2026年1月发布，实测性能**超越英伟达AGX Orin** |

> **参数说明**：天垓100采用7nm制程，集成240亿晶体管，2.5D CoWoS封装，板级功耗250W。智铠100峰值带宽800GB/s，支持128路视频解码。

天垓100的FP16算力为147 TFLOPS，与英伟达A100（312 TFLOPS）相比约为其47%,天垓150提升至192 TFLOPS，性能进一步逼近，智铠100的INT8算力达384 TOPS，在国产推理卡中处于领先水平。

#### 2. 出货量与市场表现

截至2026年1月（上市时披露），天垓系列产品累计交付**超5.2万片**，其中2025年上半年出货1.57万片，同比增长2.3倍。

训练卡（天垓系列）：年出货量接近**2万颗**，推理卡（智铠系列）：2026年预计营收占比将超70%，成为增长主力。

2024年营收超6亿元，2026年目标**10亿元**，2026年1月8日在港交所上市（股票代码：9903.HK），开盘上涨31.54%。

已服务**超300家客户**，完成**超1000次部署应用**，主要客户为计算中心、运营商、四大行、中国人寿、瑞幸咖啡（数千家门店）, 已在20个车路协同试点城市落地.



### 6.9.6摩尔线程

摩尔线程是中国首家推出**全功能GPU**并实现量产的公司，2025年12月刚在科创板上市（被称为“国产GPU第一股”），其创始人兼CEO张建中曾担任英伟达全球副总裁，拥有15年的GPU行业经验。目前市值已超过2800亿元。

#### 1. 核心产品与性能参数

摩尔线程的主力产品是**MTT S5000**旗舰级AI训推一体智算卡，2024年推出，近期首次公开了详细参数：

| 技术维度 | 详细参数 | 对比参考 |
| :--- | :--- | :--- |
| **关键算力** | **1000 TFLOPS** (FP8) | 达到**PFLOPS级别**，首次摸到国际顶尖门槛 |
| **显存规格** | **80GB** HBM2e，带宽**1.6TB/s** | [与英伟达H100相当](https://www.eet-china.com/mp/a474853.html) |
| **互联带宽** | **784GB/s** (卡间) | 支持万卡集群高效协同 |
| **计算精度** | 支持从**FP8到FP64**的全精度计算 | 国内最早原生支持FP8的训练GPU之一 |

**性能实测表现**：

智源研究院基于S5000千卡集群训练具身大脑模型RoboBrain 2.5，训练损失值与H100集群的差异仅为**0.62%**，整体训练效果甚至实现小幅超越，在互联网厂商的端到端任务测试中，S5000性能约为英伟达H20的**2.5倍**，单卡Prefill吞吐超**4000 tokens/s**，Decode吞吐超**1000 tokens/s**，刷新国产GPU推理纪录。

摩尔线程保持**每年一代架构迭代**，2025年已推出第五代“花港”架构，算力密度提升50%、能效比提升10倍；基于该架构的“华山”（AI训推）和“庐山”（图形渲染）芯片预计2026年面市。

#### 2. 市场表现与出货量

2025营收**15亿元**，同比增长超2倍，净亏损约**10.7亿元**，同比收窄37%，亏损趋势改善，综合毛利率达**70.71%**，已超过海光信息、寒武纪等同行，管理层预计最早**2027年实现盈利**。

#### 3.  软件生态：MUSA架构

与专注AI训练的厂商不同，摩尔线程走的是**“全功能GPU”路线**——单芯片同时支持AI计算、图形渲染、物理仿真和科学计算。

核心技术是自研的**MUSA（元计算统一架构）**：
- 具备与CUDA生态的**兼容性**，开发者可以较低成本迁移现有代码
- TileLang原生算子单元测试覆盖率**超80%**，显著降低移植成本
- 开发者社区规模近**20万人**，已进入全国200多所高校

### 6.9.7 阿里平头哥

阿里平头哥是阿里巴巴旗下的芯片设计公司，也是目前国产AI芯片阵营中**累计出货量领先**的玩家之一。它的核心特点是“**自研自用+对外输出**”，不仅支撑着阿里云和通义千问大模型，也向400多家外部企业提供算力。

####  1. 核心产品

| 产品系列 | 类型 | 定位与应用 | 关键特点 |
| :--- | :--- | :--- | :--- |
| **真武PPU** | AI训推一体芯片 | **当前主力**，用于大模型训练/推理、自动驾驶 | [对标英伟达H20，已在阿里云部署多个万卡集群](https://m.chinaaet.com/article/3000176025) |
| **含光800** | AI推理芯片 | 首款自研AI芯片，用于搜索推荐、图像识别 | [2019年发布，当时全球性能最强AI推理芯片之一](https://m.21jingji.com/article/20260122/herald/cc5ffbe04aae1a3c699f4d6c64c996a6.html) |
| **倚天710** | ARM服务器CPU | 阿里云数据中心通用计算 | 5nm工艺，业界性能最强的ARM服务器芯片之一 |
| **镇岳510** | SSD主控芯片 | 企业级存储 | 自研存储控制，可对冲存储芯片涨价风险 |
| **羽阵系列** | RFID芯片 | 物联网、供应链管理 | 已应用于麦当劳等场景，累计出货数亿颗 |
| **玄铁系列** | RISC-V处理器IP | 边缘计算、IoT设备 | 开源生态，累计授权超45亿颗 |

#### 2. 主力产品：“真武”PPU

这是平头哥当前最受关注的AI芯片，也是其市场地位的核心支撑。
...
- **性能对标**：
  - 整体性能**超过英伟达A800**和主流国产GPU
  - **与英伟达H20相当**
  - 针对Transformer架构优化，性价比优势明显

- **部署情况**：
  - 已在阿里云建成**多个万卡集群**
  - 支撑 **通义千问（Qwen）** 大模型的训练与推理

#### 3. 出货量与客户

**官方披露数据（2026年3月）**显示截至2026年2月，AI芯片累计规模化交付**47万片**，其中**60%以上**服务于[外部商业化客户](https://finance.eastmoney.com/a/202603193677610428.html)，已支持**400多家**企业客户的AI任务

[第三方口径（IDC 2026年Q1）](https://36kr.com/p/3680138297257857)：

累计出货量已突破**60万片**,在国产AI芯片厂商中跃居**第二**，仅次于华为昇腾

**主要客户**阿里自身（通义千问）、新浪微博等，金融主要是多家头部金融机构（风控模型训练），国家电网，中科院，小鹏汽车等（自动驾驶算法迭代）。

### 6.9.8 华为昇腾

华为昇腾（Ascend）系列是华为自研的AI处理器，采用**达芬奇（DaVinci）架构**，是专门的AI计算加速卡，算是NPU，是**目前国内落地规模最大、生态最成熟的AI算力解决方案**。

| 芯片型号 | **定位与场景** | **关键算力 (FP16)** | **关键特点** |
| :--- | :--- | :--- | :--- |
| **昇腾310** | **边缘推理/低功耗终端** | 8 TFLOPS | 12nm工艺，功耗仅8W，用于安防、工业检测等 |
| **昇腾610** | **智能驾驶** | 100 TFLOPS | 7nm制程，用于华为MDC智能驾驶平台 |
| **昇腾910B** | **云端AI训练/推理**主力 | 约**280-320 TFLOPS** | 7nm工艺，对标英伟达A100，已大规模部署 |
| **昇腾910C** | **云端AI训练/推理**旗舰 | 约**800 TFLOPS** | 双die封装，能效比优于H100，当前旗舰产品 |
| **昇腾950PR/DT** | **新一代推理/训练** (2026年推出) | **1 PFLOPS (FP8)** | 首发搭载华为自研HBM内存，SIMT架构升级 |
| **昇腾960** | **旗舰训练芯片** (规划中) | **2 PFLOPS (FP8)** | 支持自研HiF4格式，算力较950系列翻倍 |
| **昇腾970** | **下一代训练芯片** (规划中) | **4 PFLOPS (FP8)** | 算力再次翻倍，预计2028年推出 |

> **说明**：上表算力数据为官方宣称峰值，实际性能受软件优化影响。910系列已大规模商用，950及以上为公布路线图。

在一些算力平台如 [AutoDL](https://www.autodl.com/market/list) 上面可以租到 910B2x鲲鹏920 / 64 GB。

AutoDL平台有[性能实测](https://www.autodl.com/docs/gpu_perf/)，我们可以看到华为昇腾910B2x的具体算力：
...
昇腾采用**CANN（异构计算架构）**，相当于英伟达的CUDA，目前已**全面开源开放**，提供从算子开发到模型调优的全栈工具。并且**PyTorch 2.1版本已原生支持昇腾**，开发者可以直接通过`pip install`安装使用，无需复杂适配。同时兼容TensorFlow、飞桨（PaddlePaddle）及华为自研的MindSpore框架。**HuggingFace社区的Transformers库已原生支持昇腾**，下载模型后即可直接运行。昇腾社区ModelZoo 2.0也提供了数百个预训练模型。


根据多方行业数据及预测，华为昇腾在2024年已成为国产AI芯片的出货量冠军，并预计在2026年进一步扩大领先优势。

#### 2. 昇腾出货量数据

| 时间/预测期 | 出货量/产量 | 数据性质与说明 | 数据来源 |
| :--- | :--- | :--- | :--- |
| **2024年** | **64万片** | **已实现出货量**。在当年的中国AI芯片市场中，华为是唯一能与英伟达抗衡的本土厂商。 | IDC（引用自摩尔线程招股书） |
| **2025年** | **70万颗以上** | **行业预测**。分析师认为这显示了华为在制裁下的强劲需求和生产韧性。 | 瑞穗证券 |
| **2026年 (总体)** | **80万 - 85万片** | **行业预测**。报告指出，华为预计在2026年交付该数量的AI芯片。 | 电子工程专辑 |
| **2026年 (生产计划)** | **160万颗** (晶圆级) | **生产计划目标**。消息人士称，华为计划将昇腾产品线的总产量提升至160万颗（晶圆级）。这代表了其产能目标，但不等同于最终出货量。 | 星岛环球网 / 彭博社 |
| **2026年 (旗舰款910C)** | **60万片** | **生产计划目标**。计划产量约为2025年的两倍，显示华为正集中资源提升最先进芯片的产能。 | 星岛环球网 / 彭博社 |

出货量增长的背后，是昇腾在中国AI芯片市场地位的跃升。根据知名研究机构**伯恩斯坦（Bernstein Research）** 的预测，中国AI芯片市场格局将在2026年发生根本性逆转：

**华为昇腾**市场份额预计将大幅增长至**50%**，成为中国市场的绝对领导者。由于美国的制裁政策，**英伟达**：份额则可能从2025年的高位急剧萎缩至**8%**。
